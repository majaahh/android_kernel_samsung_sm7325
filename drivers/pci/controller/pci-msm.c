...
static u32 pcie_get_cur_link_bw(u32 rc_idx, u32 *speed, u32 *width)
{
...
static bool is_esoc_alive(struct msm_pcie_dev_t *dev)
{
	...
	if (is_subsystem_crash(dev->esoc_name))
		return false;

	if (is_subsystem_online(dev->esoc_name) == 0)
		return false;
...
static bool is_need_pcie_error_oops(struct pci_dev *dev, struct msm_pcie_dev_t *msm_dev)
{
	...
	if (msm_dev->oops_after_ready && dev && !dev->drv_probe_ready)
		return false;

	if (msm_dev->pcie_error_wq) {
		if (delayed_work_pending(&msm_dev->pcie_error_dwork)) {
			PCIE_ERR(msm_dev, "RC%d pcie_error_dwork is alreay triggered. skip oops.\n", msm_dev->rc_idx);
			return false;
		}
	}
...
#ifdef CONFIG_SEC_PCIE
	if (dev->subpcb_det_upper_gpio_level &&
			!dev->subpcb_det_lower_gpio_level) {
		msm_pcie_write_reg(dev->phy,
				0x338, 0x1F);
	}
#endif
					...
					u16 target_link_speed)
{
	...
	if (pcie_dev->cx_vreg && pcie_dev->cx_vreg->max_v)
		regulator_set_voltage(pcie_dev->cx_vreg->hdl,
					bw_scale->cx_vreg_min,
					pcie_dev->cx_vreg->max_v);

	if (pcie_dev->mx_vreg && pcie_dev->mx_vreg->max_v)
...
static irqreturn_t handle_aer_irq(int irq, void *data)
{
	...
	dev->aer_irq_counter++;
	set_bit(PCIE_ERROR_AER, &dev->pcie_error);
#endif
...
#ifdef CONFIG_SEC_PCIE_AER
		pci_read_config_dword(pcidev, aer_cap + PCI_ERR_CAP,
				      &ep_cap_ctrl);

		for (i = 0; i < 4; i++) {
			pci_read_config_dword(pcidev, aer_cap + PCI_ERR_HEADER_LOG + 4 * i,
				      &ep_header_log[i]);
			pci_read_config_dword(pcidev, aer_cap + PCI_ERR_TLPPREFIX_LOG + 4 * i,
				      &ep_tlpprefix_log[i]);
		}
...
static int msm_pcie_probe(struct platform_device *pdev)
{
	...
	pcie_dev->oops_after_ready =
		of_property_read_bool((&pdev->dev)->of_node,
				"oops-after-ready");

	PCIE_DBG(pcie_dev, "RC%d oops-after-ready is %s set.\n",
		pcie_dev->rc_idx, pcie_dev->oops_after_ready ? "" : "not");
	...
	if (pcie_dev->rc_idx == 0) {
		...
		pcie_dev->pcie_error_defer_ms = 5000;
				...
				u16 target_link_width)
{
	...
	if (set_link_width) {
		...
		if (ret) {
			PCIE_ERR(pcie_dev, "[%d] PCIe: RC%d: exit - msm_pcie_set_link_width(%d)\n",
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (dev->mhi) {
		count += scnprintf(buf + count, PAGE_SIZE - count,
				"L0s: %u L1: %u L1.1: %u L1.2: %u\n",
				readl_relaxed(dev->mhi +
						PCIE20_PARF_DEBUG_CNT_IN_L0S),
				readl_relaxed(dev->mhi +
						PCIE20_PARF_DEBUG_CNT_IN_L1),
				readl_relaxed(dev->mhi +
				    ...
				    const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		...
		dev_err(&client->dev, "Error: No device of_node found\n");
		return -ENODEV;
	}
			...
			void *user, void *data, u32 options)
{
	...
	if (dev) {
		...
		if (rc_linkup) {
			...
			PCIE_DBG(pcie_dev,
				"PCIe RC%d: device ID and vender ID of EP are 0x%x.\n",
				pcie_dev->rc_idx, ep_linkup);
			if (ep_linkup == PCIE_LINK_DOWN) {
				PCIE_ERR(pcie_dev,
					"PCIe RC%d: The link is not really up. device ID and vender ID of EP are 0x%x.\n",
					pcie_dev->rc_idx, ep_linkup);
			}
