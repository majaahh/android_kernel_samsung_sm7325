...
#if defined(CONFIG_DISPLAY_SAMSUNG)
	LCD_INFO(vdd, "%s %s --\n", power_mode == SDE_MODE_DPMS_LP1 ? "LP1" :
		power_mode == SDE_MODE_DPMS_LP2 ? "LP2" : "NO_LP", rc ? "failed" : "successful");
#endif
...
#if defined(CONFIG_DISPLAY_SAMSUNG)
	// ksj TODO: check if should replace with ss_early_display_init() like sm8350...
...
int dsi_display_enable(struct dsi_display *display)
{
	...
	if (is_skip_op_required(display)) {
		...
		mode = display->panel->cur_mode;
		//if ((mode->priv_info->dsc_enabled) && (!vdd->mipi_header_modi)) { // ksj TODO: compare to 8350 and check if need to add mipi_header_modi
